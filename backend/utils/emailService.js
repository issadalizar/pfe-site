...
export const sendReturnRequestNotificationEmail = async (returnRequest, order) => {
  try {
    ...
    const orderNumber = order._id.toString().slice(-8).toUpperCase();
    const requestId = returnRequest._id.toString().slice(-8).toUpperCase();
    const typeLabel = returnRequest.type === 'retour' ? 'Retour' : 'Échange';
    const typeBg = returnRequest.type === 'retour' ? '#fee2e2' : '#dbeafe';
    const typeColor = returnRequest.type === 'retour' ? '#991b1b' : '#1e40af';

    const itemsList = returnRequest.items.map(item =>
      `<tr><td style="padding:8px 12px;border-bottom:1px solid #f1f5f9;font-size:13px;">${item.productName}</td><td style="padding:8px 12px;border-bottom:1px solid #f1f5f9;text-align:center;font-size:13px;">${item.quantity}</td></tr>`
    ).join('');
...
<body style="margin:0;padding:0;background:#f1f5f9;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
<div style="max-width:580px;margin:0 auto;padding:24px 16px;">
  <div style="background:linear-gradient(145deg,#dc2626,#991b1b);border-radius:12px 12px 0 0;padding:24px;text-align:center;">
    <h2 style="margin:0;color:white;font-size:20px;">Demande de ${typeLabel}</h2>
    <p style="margin:6px 0 0;color:rgba(255,255,255,0.8);font-size:13px;">Commande #${orderNumber} | Ref: ${requestId}</p>
  </div>
  <div style="background:white;padding:24px;border-radius:0 0 12px 12px;box-shadow:0 4px 15px rgba(0,0,0,0.06);">
    <div style="background:${typeBg};border:1px solid ${typeColor}30;border-radius:8px;padding:12px;margin-bottom:20px;text-align:center;font-size:13px;color:${typeColor};font-weight:600;">
      Type: ${typeLabel} - En attente de traitement
    </div>
    <h4 style="margin:0 0 8px;font-size:14px;color:#0f172a;">Client</h4>
    <p style="margin:0 0 16px;font-size:13px;color:#64748b;">${order.shippingInfo.fullName} - ${order.shippingInfo.email}<br>Tel: ${order.shippingInfo.phone}</p>
    <h4 style="margin:0 0 8px;font-size:14px;color:#0f172a;">Motif</h4>
    <p style="margin:0 0 16px;font-size:13px;color:#64748b;background:#f8fafc;padding:12px;border-radius:8px;">${returnRequest.reason}</p>
    <h4 style="margin:0 0 8px;font-size:14px;color:#0f172a;">Articles concernes</h4>
    <table style="width:100%;border-collapse:collapse;margin-bottom:16px;">
      <thead><tr style="background:#f8fafc;"><th style="padding:8px 12px;text-align:left;font-size:11px;color:#64748b;text-transform:uppercase;">Produit</th><th style="padding:8px 12px;text-align:center;font-size:11px;color:#64748b;text-transform:uppercase;">Qte</th></tr></thead>
      <tbody>${itemsList}</tbody>
    </table>
    <div style="text-align:center;margin-top:24px;">
      <a href="${process.env.FRONTEND_URL || 'http://localhost:5173'}/return-requests" style="display:inline-block;background:linear-gradient(145deg,#dc2626,#991b1b);color:white;text-decoration:none;padding:12px 28px;border-radius:50px;font-size:13px;font-weight:600;">
        Gerer les demandes
      </a>
    </div>
  </div>
    ...
    const info = await transporter.sendMail({
      from: `"UniVerTechno+" <${process.env.SMTP_USER || 'noreply@univertechno.com'}>`,
      to: adminEmail,
      subject: `Demande de ${typeLabel} - Commande #${orderNumber}`,
      html
    });

    console.log(`Email notification retour envoye: ${info.messageId}`);
    return { success: true, messageId: info.messageId };
  } catch (error) {
    console.error('Erreur envoi email notification retour:', error);
    return { success: false, error: error.message };
  }
};
