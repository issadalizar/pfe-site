          ...
          <div className="card shadow-sm">
            ...
            <div className="card-body">
                ...
                <table className="table table-hover align-middle">
                  ...
                  <tbody>
                    {users.map((u) => (
                      <tr key={u._id}>
                        <td>
                          <strong>{u.client_code}</strong>
                        </td>
                        <td>{u.client_name}</td>
                        <td>{u.article}</td>
                        <td className="text-muted small">{u.description}</td>
                        <td>{new Date(u.date).toLocaleDateString("fr-FR")}</td>
                        <td>{u.facture_num}</td>
                        <td className="text-center">
                          <span className="badge bg-info">{u.quantite}</span>
                        </td>
                        <td className="text-end fw-bold">
                          {Number(u.montant || 0).toFixed(2)} TND
                        </td>
                        <td>
                          <span
                            className={`badge bg-${
              ...
              <div className="d-flex flex-wrap gap-3 text-muted">
                ...
                <span>
                  <strong>Montant total :</strong> {totalMontant.toFixed(2)} TND
                </span>
              </div>
            </div>
          </div>
